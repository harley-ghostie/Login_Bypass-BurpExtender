    ...
    public void initialize(MontoyaApi api) {
        ...
        api.scanner().registerScanCheck(new ScanCheck() {
            ...
            public AuditResult passiveAudit(HttpRequestResponse baseRequestResponse) {
                ...
                // Regex para identificar padrões suspeitos de login client-side
                ...
                if (snippet != null) {
                    AuditIssue issue = new CustomAuditIssue(
                        ...
                        (snippet.contains("getCookie") || snippet.contains("setCookie") ?
                            "document.cookie = 'login=admin'; location.reload();" :
                            snippet) +
                        "\n\nSe houver redirecionamento ou acesso ao painel, a autenticação está ocorrendo apenas no lado cliente.",
                        "Implemente autenticação e controle de acesso exclusivamente do lado servidor.",
